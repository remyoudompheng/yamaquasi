...
//!
//! For class group computations, matrix dimension will usually be less than
//! 65536, so we use 16-bit indices, and determinants will be less than
//! 128kb large.
...
type IntLarge = BInt<1024>;
type UintLarge = BUint<1024>;
        ...
        for i in 0..self.size {
            ...
            unsafe {
                ...
                for idx in idx1..idx2 {
                    let (j, mij) = *self.values_x.get_unchecked(idx);
                    let vj = v.get_unchecked(j as usize);
                    x = x + I::cast_from(mij as i32) * I::cast_from(*vj);
                }
...
fn crt(modp: &[u64], primes: &[u64]) -> IntLarge {
    match modp.len() {
        0 => IntLarge::ZERO,
        1 => modp[0].into(),
        2..=16 => IntLarge::cast_from(_crt::<16>(modp, primes)),
        ...
        257..=512 => IntLarge::cast_from(_crt::<512>(modp, primes)),
        _ => IntLarge::cast_from(_crt::<1024>(modp, primes)),
    }
}
...
fn _crt<const N: usize>(modp: &[u64], primes: &[u64]) -> BInt<N> {
    ...
    let mid = revprods.len() / 2; // mid >= 1
    let prod = fwdprods[mid].checked_mul(revprods[mid - 1]).unwrap();
    det %= prod;
    if det > (prod >> 1) {
